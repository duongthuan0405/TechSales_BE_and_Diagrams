...
Write-Host "Đang ghi đè file chính thức và dọn dẹp các file -re.txt..." -ForegroundColor Cyan

for ($i=1; $i -le 60; $i++) {
    $folder = "UC{0:D2}" -f $i
    $reFile = "AD{0:D2}-re.txt" -f $i
    $mainFile = "AD{0:D2}.txt" -f $i
...
Write-Host "Thành công! Đã áp dụng và xóa $count file -re.txt" -ForegroundColor Green
