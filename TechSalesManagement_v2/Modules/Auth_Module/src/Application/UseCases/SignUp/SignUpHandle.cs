...
namespace Auth_Module.src.Application.UseCases.SignUp
{
    public class SignUpHandle : IRequestHandler<SignUpCommand, SignUpCommandResponse>
    {
        ...
        {
            return await _executeAtomically.ExecuteAtomically<SignUpCommandResponse>(() => MainTask(request, cancellationToken));
        }

        private async Task<SignUpCommandResponse> MainTask(SignUpCommand request, CancellationToken cancellationToken)
        {
